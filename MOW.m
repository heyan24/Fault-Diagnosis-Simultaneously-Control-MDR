...
for t=1:rep1
    ...
    for k=1:rep2
        ...
        D1=(p1*g1+p2*g2)./(p0*g0+p1*g1+p2*g2);
        [D1_sort,Ind]=sort(D1);
        SUM=sum(D1);
        for j=1:m
        	result(j)=sum(D1_sort(1:j))/SUM;
        end
        cut=find(result-alpha>=0,1);
        delta=zeros(1,m);
        delta(Ind(cut:m))=(X(Ind(cut:m))>0)*1+(X(Ind(cut:m))<0)*-1;
...
MOW_result=[mean(MDR1)',mean(MDR2)',mean(MDR)',mean(EFP)'];
csvwrite("../results/MOW2_prop2.csv",MOW_result);
%end
toc
load train
sound(y,Fs)